...
export default function AuthenticatedLayout({ header, children }) {
    ...
    return (
        <div className="min-h-screen bg-slate-900 bg-[radial-gradient(ellipse_at_top_right,_rgba(139,92,246,0.12),_transparent_50%),radial-gradient(ellipse_at_bottom_left,_rgba(236,72,153,0.08),_transparent_50%)]">
            <nav className="sticky top-0 z-30 border-b border-slate-700/50 bg-slate-900/90 backdrop-blur-xl">
                <div className="mx-auto max-w-7xl px-3 sm:px-4 lg:px-6">
                    <div className="flex h-12 items-center justify-between">
                        <div className="flex items-center gap-2">
                            <Link href={route('dashboard')} className="flex shrink-0 items-center gap-2">
                                <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-gradient-to-br from-violet-600 to-fuchsia-600 text-white shadow-sm">
                                    <ApplicationLogo className="block h-4 w-4 fill-current text-white" />
                                </div>
                                <span className="hidden text-sm font-extrabold text-white sm:inline">LR JPRD</span>
                            </Link>

                            <div className="ml-4 hidden items-center gap-0.5 sm:flex">
                                {navItems.map((item) =>
                                    canAccess(item.key) && (
                                        <NavLink key={item.key} href={route(item.href)} active={route().current(item.routePattern)}>
                                            {item.label}
                                        </NavLink>
                                    )
                                )}
                                {isMasterAdmin && (
                                    <NavLink href={route('admin.access.index')} active={route().current('admin.access.*')}>
                                        Akses
                                    </NavLink>
                                )}
                            </div>
                        ...
                        <div className="flex items-center gap-2">
                            <div className="hidden sm:block">
                                <Dropdown>
                                    <Dropdown.Trigger>
                                        <button className="flex items-center gap-2 rounded-lg border border-slate-700 bg-slate-800/80 px-2.5 py-1.5 text-xs font-semibold text-slate-300 shadow-sm transition hover:border-violet-500/50 hover:text-violet-300">
                                            {user.avatar_url ? (
                                                <img src={user.avatar_url} alt={user.name} className="h-6 w-6 rounded object-cover" />
                                            ) : (
                                                <div className="flex h-6 w-6 items-center justify-center rounded bg-gradient-to-br from-violet-600 to-fuchsia-600 text-[10px] font-bold text-white">{userInitial}</div>
                                            )}
                            ...
                            <button onClick={() => setShowingNavigationDropdown((prev) => !prev)}
                                className="inline-flex items-center justify-center rounded-lg p-1.5 text-slate-400 transition hover:bg-slate-800 hover:text-white sm:hidden">
                ...
                <div className={(showingNavigationDropdown ? 'block' : 'hidden') + ' border-t border-slate-700/50 bg-slate-900 sm:hidden'}>
                    <div className="space-y-0.5 px-2 py-2">
                        {navItems.map((item) =>
                            canAccess(item.key) && (
                                <ResponsiveNavLink key={item.key} href={route(item.href)} active={route().current(item.routePattern)}>
                                    {item.label}
                                </ResponsiveNavLink>
                            )
                        )}
                        {isMasterAdmin && (
                            <ResponsiveNavLink href={route('admin.access.index')} active={route().current('admin.access.*')}>Akses Pengguna</ResponsiveNavLink>
                        )}
                    </div>
                    <div className="divider px-3 py-2">
                        <div className="flex items-center gap-2.5">
                            {user.avatar_url ? (
                                ...
                                <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-violet-600 to-fuchsia-600 text-xs font-bold text-white">{userInitial}</div>
                            )}
                            <div>
