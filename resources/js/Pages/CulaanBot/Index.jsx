...
export default function CulaanBotIndex({ filters, summary, udms, localities, voters, available_cula_codes = [] }) {
    ...
    const [detailVoterIsSearch, setDetailVoterIsSearch] = useState(false);
    const closeDetail = () => { setDetailVoter(null); setDetailVoterIsSearch(false); };
                        ...
                        <div className="grid gap-3">
                            {rows.map((voter, index) => {
                                ...
                                if (isSearchResult && voter.is_marked) {
                                    return (
                                        <button key={voter.id} type="button" onClick={() => { setDetailVoter(voter); setDetailVoterIsSearch(true); }}
                    ...
                    <div className="mx-4 w-full max-w-md rounded-xl bg-white p-5 shadow-xl" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
                        ...
                        {detailVoterIsSearch && (
                            <div className="mt-4 flex flex-wrap gap-2 border-t border-slate-100 pt-4">
                                {!detailVoter.avatar_url && !detailVoter.is_manual && (
                                    <>
