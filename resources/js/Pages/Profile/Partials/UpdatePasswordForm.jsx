    ...
    const PasswordField = ({ id, label, value, onChange, inputRef, autoComplete, error, visibleKey }) => (
        <div>
            ...
            <div className="relative mt-1">
                ...
                <TextInput id={id} ref={inputRef} value={value} onChange={onChange} type={visible[visibleKey] ? 'text' : 'password'} className="input-field pl-10 pr-10 text-sm" autoComplete={autoComplete} />
                <button type="button" onClick={() => setVisible((current) => ({ ...current, [visibleKey]: !current[visibleKey] }))} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 transition hover:text-slate-600">
