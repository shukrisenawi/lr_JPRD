...
function KeyCell({ k, copy: copyFn, copiedKey }) {
    ...
    const key = typeof k.key === 'string' ? k.key : null;

    if (!key) {
        return <span className="whitespace-nowrap text-[10px] font-semibold text-amber-600" title="Kunci ini tidak boleh dibaca. Padam dan cipta kunci baharu.">Tidak tersedia</span>;
    }

    return (
        <div className="flex items-center gap-1">
            <span className="min-w-0 truncate font-mono text-slate-500" style={{ maxWidth: '14ch' }}>{key.slice(0, 12) + '...'}</span>
            {justCopied ? (
                <span className="whitespace-nowrap text-[10px] font-bold text-blue-600">Telah disalin</span>
            ) : (
                <button type="button" onClick={function () { copyFn(key, k.id); }} className="shrink-0 rounded p-0.5 text-slate-400 hover:text-blue-600" title="Salin kunci">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="h-3.5 w-3.5" aria-hidden="true"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>
                </button>
            )}
        </div>
    );
