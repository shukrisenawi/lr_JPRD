...
function getRaceCount(breakdown, names, metric = 'total') {
    for (const r of breakdown ?? []) {
        if (names.includes(r.code.toUpperCase())) return r[metric] ?? r.total ?? 0;
    }
    return 0;
}
...
export default function Laporan({ report, culaan_message = '', pemilih_report = null, udm_snapshot = null, udm_snapshot_meta = null, recent_logins = [], ahli_pas_stats = null }) {
    ...
    const allUdmTableRows = useMemo(() => [...report.by_dm]
        ...
        .map(row => {
            ...
            return {
                ...
                JP: row.active_total ?? row.total ?? 0,
                L: row.active_male ?? row.male ?? 0,
                P: row.active_female ?? row.female ?? 0,
    ...
    const groupH = {
        ...
        aggregate: 'bg-violet-100 text-violet-900',
    ...
    const dmCols = [
        ...
        { key: 'Mati', label: 'MT', format: (v, r) => fmtDiff(v, diffMap[r.key]?.Mati), headerClass: groupH.party, cellClass: groupC.party },
